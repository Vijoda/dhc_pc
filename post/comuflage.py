...
def meta_work(input_file):
    if not exists('meta_comuflage.csv'):
        with open('meta_comuflage.csv', 'w', newline='') as csvfile:
            csv_writer = csv.writer(csvfile)
            print(input_file.columns)
            csv_writer.writerow(list(input_file.columns))
            csvfile.close()

    meta_file = pd.read_csv('meta_comuflage.csv', index_col=False).fillna('')
...
def creating_input(input_file,output_file):
    ...
    input_file.to_csv(settings.DHCP_Post_Comuflage_input, index=False)
def output_process(row,text_type,output_url,visible_text,nick_name_value):
    try:
        print('Output')
        Post_processing='COMUFLAGE'
...
def create_directory_structure(url):
    ...
    path=path.replace('/','\\')
    print(path)
    domain_directory = os.path.join(script_directory,'HTML_DATA', domain)
    os.makedirs(domain_directory, exist_ok=True)

    if path:
        subfolder_path = os.path.join(domain_directory, path)
...
def extract_visible_text(url, query_1,query_2,firstname,lastname,middle_name,row):
    try:
        ...
        with sync_playwright() as p:
            ...
            page.goto(url, wait_until='load', timeout=600000)
            output_url = page.url
            html = page.content()
            time.sleep(2)
            # print(html)
...
def process_url(row, execution_id):
    ...
    #             row['input_type']]
        ...
        print(ex)
    with open('meta_comuflage.csv', 'a', newline='') as csvfile:
        csv_writer = csv.writer(csvfile)
        csv_writer.writerow(tuple(row.values()))
        csvfile.close()
def start_camuflage(input_file,output_file,num_threads):
    global nick_data
